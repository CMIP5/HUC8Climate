...
if (!function_exists('wof_GetSiteInfo')) {
	...
	function wof_GetSiteInfo($fullSiteCode) {
		...
		if (strpos($fullSiteCode, ":")) {
			$split = explode(":", $fullSiteCode);
			$shortcode = $split[1];
		} else {
			$shortcode = $fullSiteCode;
		}
...
if (!function_exists('db_GetMethodInfo')) {
...
if (!function_exists('db_GetSeriesCatalog')) {
	...
	function db_GetSeriesCatalog() {
	   ...
	   VariableUnitsID, NoDataValue, IsRegular, u2.UnitsName AS \"TimeUnitsName\", 
	   u2.UnitsType AS \"TimeUnitsType\", u2.UnitsAbbreviation AS \"TimeUnitsAbbreviation\", TimeUnitsID, TimeSupport, Speciation");
		$ci->db->join($units_table." u1","v.VariableUnitsID = u1.UnitsID","LEFT");
		$ci->db->join($units_table." u2","v.TimeUnitsID = u2.UnitsID");

	    $variablesResult = $ci->db->get($variables_table." v");	
	    if (!$variablesResult) {
		...
		//fetch the source
		$source = db_GetSourceByID(1, 1);
		$sourceID = '1';
		$sourceOrganization = $source['Organization'];
		$sourceDescription = $source['SourceDescription'];
		$sourceCitation = $source['Citation'];
		
		//fetch the quality control level
		$qcID = 1;
		$qcCode = '1';
		$qcDefinition = 'Quality controlled data';
	   
		//get the methods
		$methods = db_GetMethods();

	    $retVal = '<seriesCatalog>';
	
		//for all of the variables we add a series_catalog entry..
...
if (!function_exists('db_GetSiteByCode')) {
	...
	function db_GetSiteByCode($shortCode, $siteTag = "siteInfo", $siteTagType = "") {
		...
		$sites_table = get_table_name('Sites');
		$ci->db->select("SiteName, SiteID, SiteCode, Latitude, Longitude, LocalProjectionID, LocalX, LocalY, Elevation_m, VerticalDatum, State, County, Comments, PosAccuracy_m");
		$ci->db->where("SiteCode",$shortCode);	
...
if (!function_exists('db_GetSiteID')) {
	function db_GetSiteID($shortCode) {
		$ci = &get_instance();
		$sites_table = get_table_name('Sites');
		$ci->db->select("SiteID");
		$ci->db->where("SiteCode", $shortCode);
...
if (!function_exists('db_GetSites')) {
	...
	function db_GetSites($site = NULL) {
		...
		$ci->db->select("s.SiteName, s.SiteID, s.SiteCode, s.Latitude, s.Longitude, s.LocalProjectionID, s.LocalX, s.LocalY, s.Elevation_m, s.VerticalDatum, s.State, s.County, s.Comments, s.PosAccuracy_m");
...
if (!function_exists('db_GetResultWML2')) {
	...
	function db_GetResultWML2($siteCode, $variableCode, $beginTime, $endTime) {
	    ...
	    if ($numSeries == 0) {
			...
			$retVal .=  db_GetValues_OneSeriesWML2($row["SiteID"], $siteCode, $row["State"], $variableCode, $row["VariableID"], $row["MethodID"], $row["SourceID"], $row["QualityControlLevelID"], $beginTime, $endTime);
...
if (!function_exists('db_GetMethods')) {

	function db_GetMethods() {
	    $ci = &get_instance();
