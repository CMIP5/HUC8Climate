...
class Datapoint extends MY_Controller {
	...
	public function getData()
	{
		...
		{
			$result = $this->datapoints->getData3($site,$var,$method,$start,$end);
			...
			{
				$pieces = explode("T", $row['LocalDateTime']);
				$pieces2 = explode("-", $pieces[0]);
				
				$output="[Date.UTC(".$pieces2[0].",".$pieces2[1].",".$pieces2[2].",00,00,00),".$row['DataValue']."]";
	...
	public function export()
	{
		...
		{
			$result = $this->datapoints->getData3($site,$var,$method,$start,$end);
			$methodDesc = $this->datapoints->getMethodDescription($method);
			//print_r($result);
			...
			{
				$retVal .= "\"".$var['LocalDateTime'] ."\",";
				$retVal .= $var['DataValue'].",";
				$retVal .= "\"".$methodDesc."\"\n";
			}
			$outFileName = $this->datapoints->getOutputFileName($site, $var, $method);
			
			header( 'Content-Type: text/csv' );
			header("Content-Disposition: attachment; filename=$outFileName".$site.'.csv');
			//$this->load->dbutil();
