...
class Method extends MY_Model
{
	...
	{
		$selectedVarCode = '%_pr_%';
		if ($var == 2) {
			$selectedVarCode = '%_tas_%';
		}		
		$result = $this->db->query('SELECT MethodID, MethodDescription FROM methods WHERE MethodDescription LIKE "' . $selectedVarCode . '"');		
